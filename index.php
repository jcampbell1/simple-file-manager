...
// configure to any location.
$rootFolder =  '/home/username/fm-data-files';
...
if(DIRECTORY_SEPARATOR==='\\') $rootFolder = str_replace('/',DIRECTORY_SEPARATOR,$rootFolder);

if(strpos($_REQUEST['file'], '..') !== false || strpos($_REQUEST['file'], './') !== false) 
    err(403,"Forbidden");

$tmp = get_absolute_path($rootFolder . '/' .$_REQUEST['file']);    
if($tmp === false)
    err(404,'File or Directory Not Found');
    
$file = getFilename($_REQUEST['file'], $rootFolder);
    ...
    if (is_dir($file)) {
        ...
        foreach($files as $entry) if($entry !== basename(__FILE__) && !in_array(strtolower(pathinfo($entry, PATHINFO_EXTENSION)), $hidden_extensions)) {
            ...
            $result[] = [
                ...
                'size' => $stat['size'],
                'name' => basename($entry),
                'path' => preg_replace('@^\./@','',substr($i, strlen($rootFolder))),
                'is_dir' => is_dir($i),
...
function getFilename($tempFile, $baseFolder)
{
    if($tempFile)
    {
        if(strpos($tempFile, DIRECTORY_SEPARATOR) === 0)
            $tempFile = substr($tempFile, 1);
        return $baseFolder . '/' . $tempFile;
    }    
    return $baseFolder;
}
...
#last-access {display:inline-block;float:left;}
...
<div>
	<div id="last-access">
		...
		<?php  if($PASSWORD) { ?> <a href='?do=logout'> Logout</a> <?php } ?>
	</div>
   <?php if($allow_upload == true): ?>
